...
public final class CryptoUtilities {
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        if (!p.isZero()) {
            //Used to see if power is even or odd
            NaturalNumber remainder = p.divide(two);
            //Finds n to the power of
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        //[2, n – 2] for which w^2 mod n = 1, then you know n is not prime
        ...
        // [1, n – 1] for which w^n – 1 mod n ≠ 1
        NaturalNumber tempN = new NaturalNumber2(n);
        tempW = new NaturalNumber2(w);
        tempN.decrement();
        powerMod(tempW, tempN, n);
        if (!tempW.equals(one) && !witness) {
            witness = true;
        }
        return witness;
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            while (i <= 50 && isPrime) {
                NaturalNumber tempN = new NaturalNumber2(n);
                tempN.subtract(four);
                NaturalNumber temp = randomNumber(tempN);
                temp.add(two);

                isPrime = !isWitnessToCompositeness(temp, n);
                i++;
            }
        }
        return isPrime;
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        n.multiplyBy10(lastDigit);
        while (!foundPrime) {
            // odd check odds from there on
            if (lastDigit % 2 != 0) {
                n.add(two);
            } else {
                //even, check odd greater than then check odds from there on
                n.increment();
            }
            foundPrime = isPrime2(n);
